...
public class PayMyBuddyApplication implements CommandLineRunner {
    ...
    /*PayMyBuddyApplication(AuthenticationManager authenticationManager, TransactionRepository transactionRepository) {
	...
	public void run(String... args) throws Exception {
		...
		List<TransactionProjection> transactions = transactionService.getTransactionByUserId(1);
		System.out.println("transactions");
		transactions.forEach(transaction -> System.out.println(transaction.getUsername()));*/
		
		//insert transaction
		/*Transaction transaction = new Transaction();
		Optional<User> sender = userService.getUserById(2);
		Optional<User> receiver = userService.getUserById(1);
		if (sender.isPresent() && receiver.isPresent()) {
			transaction.setUserSender(sender.get());
			transaction.setUserReceiver(receiver.get());
			transaction.setDescription("test2");
			transaction.setAmount(800.0);
